...
from badger.formula import extract_variable_keys
from badger.formula_utils import stat_key_from_expr
...
class BadgerRoutinePage(QWidget):
    ...
    def set_options_from_template(self, template_dict: dict[str, Any]):
        ...
        for name in objectives_names_full:
            obj = {name: self.env_box.objectives_list_view.default_info()}
        ...
        self.env_box.objectives_list_view.show_selected_only = True
    ...
    def refresh_ui(self, routine: Routine | None = None, silent: bool = False):
        ...
        for name in objectives_names_full:
            try:
                ...
                # TypeError: self.configs["observations"] is a list
                # KeyError: env observations does not specify default values

                # Either way no default values are specified in env configs,
                # set to general defaults
    ...
    def select_env(self, i: int):
        ...
        for name in self.configs["observations"]:
            ...
            try:
                ...
                # Improve
                logger.info("no defaults for {name}")
                pass
        ...
        self.env_box.sta_table.update_items(observables, status, vocs_signal=False)
    ...
    def _compose_routine(self) -> Routine:
        ...
        with warnings.catch_warnings(record=True) as caught_warnings:
            ...
            logger.info(f"Compose routine: vocs: {routine.vocs}")
            logger.info(f"Compose routine: formulas: {routine.formulas}")
